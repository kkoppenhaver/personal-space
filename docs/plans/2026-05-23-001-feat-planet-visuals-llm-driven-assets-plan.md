...
| 11. Thumbnail lighting / capture quality | ⌛ Pending | — | **Observed in prod:** captured logbook thumbnails look a little dark. The thumbnail is the player's only keepsake of a planet, so it should look its best. Investigate the dark capture: candidates — (1) `ThumbnailCapture._snapshot` renders to an offscreen `WebGLRenderTarget` that may not inherit the live scene's clear color / tone-mapping / exposure (e.g. `renderer.toneMapping`/`toneMappingExposure` not applied to the RT pass, or sky `setClearColor` lerp not replicated); (2) capture may fire at an altitude where atmosphere/fog has darkened the frame; (3) scene lighting (ambient/directional) may be tuned for in-cockpit feel, not a framed hero shot. Deliverables: audit the capture render path vs the live render path for parity; consider a brief capture-time lighting/exposure boost or a dedicated capture clear color; possibly frame using the LLM's `thumbnail_framing_hint`. Coordinate with Phase 5's thumbnail-capture defer (same call site) so timing + lighting fixes land coherently. |
