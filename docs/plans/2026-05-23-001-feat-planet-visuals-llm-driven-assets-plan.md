...
| 5. Reveal-as-you-fly | ✅ Shipped | [#17](https://github.com/kkoppenhaver/personal-space/pull/17) | Sub-plan: `docs/plans/2026-05-24-002-...md`. **Atmosphere-entry-gated** reveal: assets mount on Tier 2 resolve but hold at `uReveal=0` until the plane crosses the atmosphere shell, then fade in (opaque `onBeforeCompile` dither-clip, shared `'planetReveal'` program, 500ms distant / 250ms late ease-out curves; re-approach + reduced-motion snap solid). Fog tint toward biome sky. Thumbnail-capture defer (1.5s ceiling + force-solid, procedural capture immediate, "CAPTURING…" pill). Prefetch + `compileAsync` warmup on Tier 2 resolve. **Atmosphere-shell-disappears-before-entry fix (B1) was DROPPED** — the DoubleSide attempt regressed the "wrapped" look; reverted to original BackSide, revisit separately. (Intent gate shipped in #14. Distance-gated mount/dispose — old sub-feature C — deferred to Phase 8.) |
| 6. Diversity guardrails + style cohesion enforcement | ✅ Shipped | [#16](https://github.com/kkoppenhaver/personal-space/pull/16) | Sub-plan: `docs/plans/2026-05-24-001-...md`. Wired `markUsed()` post-claim; per-user ring buffer + anon→user migration; per-role recency decay (12/8/5) + ring cap 32; pack-cohesion 1.3× boost off a probabilistic hero-shortlist anchor; MMR λ 0.7→0.5; BM25-fallback first-token dedupe; `__GAME.auditMaterials()` enforcement hook. Worker `/tier2/pick` now gets `{id,pack,family}` + `anchor_pack` (requires `wrangler deploy` — done). |
| 7. Poly Pizza dynamic + attribution UX | ✅ Shipped | — | **Shipped (2026-06-11), reshaped by the API ToS:** worker `/assets/search` proxy (KV-cached 7d, CC0-filtered, key never reaches browsers) — GLBs load DIRECTLY from Poly Pizza's CDN (no re-hosting: ToS forbids replicating the service); embed-on-demand dropped — Poly Pizza's own search IS the retrieval for the dynamic path, triggered only when the catalog hero match is weak (RRF < 0.02) and the concept names what should exist; picked records ride on picks as `dynamic_assets` (preserve_materials skips the cohesion recolor); per-entry creator credits in the logbook detail + `/credits.html` (names Poly Pizza with a link, per ToS) linked from the account drawer. CC-BY + non-hero slots deferred. ToS note: no commercial use above $50k/yr. |
| 8. Performance pass | ⌛ Pending | — | Hard 2-system cap on full-realization, frustum culling re-enable, ORT WASM CDN config (currently bundled to dist but served from jsdelivr at runtime — bloat is harmless but worth cleaning). **Folded in:** (a) distance-gated mount/dispose with hysteresis (old Phase 5 sub-feature C) — mount hero at distance, dispose when far, surface up close, per-asset state machine; (b) **terrain smoothness** — planets read as spiky/faceted (icosa `subdivisions=5` + `flatShading` + `amp=radius*0.10` ridge noise), so ground-snapped assets straddle coarse facets and look like they float/stab. Bumping subdivisions 5→6 (≈4× triangles + Rapier trimesh collider) and/or softening amplitude smooths it — but the collider/triangle cost is exactly what the 2-system cap + budget here must pay for, so it belongs in this pass. Note: bigger *radius* alone does NOT help (facet size + amp both scale with radius → same relative spikiness). |
| 9. LLM latency reduction | ✅ Shipped | — | **Observed in prod:** Sonnet 4.6 `/tier2/direct` ≈14s p50, Haiku 4.5 `/tier2/pick` ≈12s p50, Tier 1 ≈1s. Even with the intent gate hiding latency behind approach flight time, planets often render as procedural-fallback for several seconds before GLBs swap in. Ideas to explore: (a) streaming the direct response so retrieval + pick can start the moment the first hint tokens arrive (cuts ~3-4s); (b) splitting `/tier2/direct` into a fast "biome + density + hints" core call and a slower "atmospheric prose + landmark names" deferred call so visuals fire on the fast half; (c) mixing models — Haiku for biome/hints (~3s), Sonnet only for the narrative prose layer; (d) speculative warm-up of `/tier2/direct` on the *adjacent* system at the moment a system spawns, so the seed is already cached before the player commits; (e) per-biome cached prompt prefix that lets the worker cache-hit Anthropic's 5-min prompt cache more often. Each option has a cost/quality tradeoff worth measuring. **Shipped (2026-06-11):** (idea b/c realized via the concept spine) — the spawn-time concept already carries biome/density/keywords/premise, so retrieval + `/tier2/pick` now run IN PARALLEL with the Sonnet direct call (`directionFromConcept` adapter; serial fallback without a concept); `onPicks` callback prefetches GLBs the moment picks land. **Measured vs prod: 15.9s end-to-end (was ~26s serial).** Name-first pipeline (#26) had already delivered progressive words. Prompt caching evaluated and SKIPPED: minimum cacheable prefix is 4096 tokens (Haiku 4.5) / 2048 (Sonnet 4.6) — all our system prompts are well under; the Phase 6 cache_control on PICK_SYSTEM has been a silent no-op (harmless). Speculative adjacent-system warmup unnecessary (concepts already fire+cache at spawn). |
| 10. Asset placement + dual-axis color system | ✅ Shipped | [#15](https://github.com/kkoppenhaver/personal-space/pull/15) | **Two sub-problems observed in prod (Velundra screenshot):** <br><br>**A. Placement.** Loaded GLBs often float above the surface (windmill prop is the canonical case) or land on tilted pedestals. Root causes: (1) `buildLandmarkInstance` positions the GLB *origin* on the slot's surface point, but many GLBs have origin at center or even above-base — they need a per-asset `bbox.min.y` offset along the surface normal so the visible base actually touches the terrain. (2) Some assets are Z-up, not Y-up — needs a per-creator/per-pack rotation override on load. (3) Per-asset `scale_range` is coarse (kit-level default); a few outliers read as oversized. Deliverables: compute + cache `boundingBox` per loaded GLB; ground-snap on mount using `bbox.min.y`; per-pack axis-up override map; debug overlay (`__GAME.debugPlacement = true`) drawing bbox + surface normal + axis gizmo on every mounted asset for visual review; per-asset `scale_override` field in `catalog.source.json` for hand-tuned outliers. Stretch: physics raycast against terrain at mount time for exact contact-point grounding. <br><br>**B. Dual-axis color system.** Today every flora/rock/landmark on a planet shares one `matSet.flora`/`rock`/`landmark` color, flattening everything to the biome accent — rocks turn green on green planets instead of staying recognizably rock-like. Replace the current single-axis palette with dual-axis coloring: <br>**Axis 1 — Semantic family anchor.** Per-family base color in **OKLCH** (perceptually-uniform space): `rock` near `oklch(45% 0.03 80)` (low-chroma neutral), `flora` near `oklch(55% 0.10 140)` (mid-chroma green), `stone`, `wood`, `metal`, `bone`, `crystal`, `sand`, `structure`. ~30-line OKLCH helper, no dependency. <br>**Axis 2 — Biome shift.** Per-biome blend factor table dictates how much each family leans toward the biome accent hue: rocks lean ~30% (gray with biome tint — basalt on volcanic, frost-gray on ice), flora ~60%, landmarks ~100% (pure biome accent), hero 0% (keep GLB authored materials). 8 biomes × ~10 families = static lookup table. <br>**Per-asset HSL nudge** on top of the family/biome resolution (hue ±2°, chroma ±5%) keyed by asset id hash so instances vary subtly without breaking family identity. <br>**Catalog change:** add `family` field to `catalog.source.json` derived in `import-assets.js` from filename (`tree_*` → `flora`, `rock_*` → `rock`, `tower-*` → `structure`, `Bone_*` → `bone`). Default fallback = current matSlot logic so this is backward-compatible. <br>**Goal:** Velundra's rocks read as gray-with-bio-tint, its flora reads as green-with-bio-tint, its landmarks pop as biome accent. Same rocks on volcanic Vel Sorath read as warm-gray basalt. Identity preserved, biome context honored. |
| 11. Thumbnail lighting / capture quality | ✅ Shipped | — | Sub-plan: `docs/plans/2026-06-11-001-feat-planet-visuals-phase-11-thumbnail-capture-plan.md`. **Shipped (2026-06-11). Audit verdict on the dark capture:** candidate (1) was the bug, narrowed to color space — the capture `WebGLRenderTarget` had no `colorSpace`, so the linear→sRGB output transform (driven by the RT texture's color space since three r152) never ran; readback was linear-light bytes encoded as if sRGB, **measured at 42% of correct brightness**. Fixed with `colorSpace: SRGBColorSpace` + `samples: 4` (MSAA parity with the canvas's `antialias: true`). Candidate (2) fog/clear color was NOT a parity bug (shared renderer/scene state, both passes see it); candidate (3) exposure boost unnecessary once (1) was fixed — none added. **Also shipped — framing (11b, load-bearing per Phase 14):** dedicated capture camera (live camera never mutated; old aspect-juggling removed) that aims at the hero GLB when it's above the geometric horizon (small slack for hero height) via new `Planet.heroWorldPosition()` off the mounted hero slot, and roll-levels to planet-radial up either way; the camera position never moves — the keepsake stays "what I saw from the cockpit". `thumbnail_framing_hint` deliberately stays unused (free prose → camera math is unverifiable; the deterministic hero subject IS the premise's subject). New parity bench: `__GAME.testThumbnail()` overlays capture vs live side by side. Phase 5 defer logic untouched. |
| 12. World archetypes + seed-derived sparks | ✅ Shipped | — | Sub-plan: `docs/plans/2026-06-10-001-feat-planet-visuals-phase-12-world-archetypes-plan.md`. **Problem:** worlds feel same-y — Tier 2 gets `{seed, radius}` only (mode collapse), the prompt's own theme examples anchor outputs, sea level is hard-coded at the 42% quantile (an "ocean" planet is 58% land), and the catalog has zero creatures. **Fix:** deterministic seed-derived archetype roll (`src/world/Archetypes.js`, weighted table: ocean-world / shipwreck-graveyard / ruled-by-creatures / ruin-field / necropolis / monolith-world / …) carrying terrain params + composition rules + a Tier 2 prompt spark; parametric `seaLevelQuantile`/`ampScale` in TerrainGen (unhardcode the 0.42 couplings in Landmarks/Features); CC0 creature packs (Quaternius animals, Kenney animals) + new `creature` role/slots. Split: 12a archetypes+terrain+prompts (existing assets) — **shipped**: 13 archetypes + twists, parametric sea level/amplitude, open-water float heroes (pirate ships scale-overridden), composition clamps, prompt rewrite (anchor examples deleted), biome soft-filter, `__GAME.rollStats`. Requires `wrangler deploy`. 12b shipped: 17 Quaternius animals (creature role end-to-end, inhabitant_hints, dynamic pick slots, herd scatter via creatureBudget, ruled-by-creatures archetype). Direction note: archetype tables likely demoted to concept-pipeline inputs in a future phase. |
| 13. Placement engine (orientation, grounding, clustering) | ✅ Shipped | — | Sub-plan: `docs/plans/2026-06-10-002-feat-planet-visuals-phase-13-placement-engine-plan.md`. **13a audit correction:** all 13 packs are Y-up (PACK_AXIS_UP stays empty); the real bugs were the clamped ground-snap (kenney_space authored above origin → floated wholesale; now SIGNED offset) and corner pivots (now lateral recenter). Plus per-family terrain-normal blend/slope gates/embed, slot flatness preference, exact-vertex slot heights, `?contactSheet=1` + minimal `?placementLab=1` benches. **Problem:** Phase 10 shipped placement *infrastructure* but `PACK_AXIS_UP` is still empty (no pack ever audited — Z-up kits mount sideways), orientation uses radial up instead of terrain normal (props lean out of hillsides), no slope/band gating, multi-mesh GLBs scatter only their first mesh, scale trusts kit units, and round-robin scatter reads as uniform confetti. **Fix:** `?contactSheet=1` audit tool → populate axis-up + per-asset overrides; per-family terrain-normal blend + slope gates; embed factor + sampler-height snap; bbox-normalized scale targeting; clustered band-aware scatter with landmark exclusion zones + geometry merge. Split: 13a audit+orientation correctness, 13b scale+clustering. **13b shipped:** bbox-normalized scale (per-role TARGET_HEIGHT, kit units cancel out), clustered band-aware scatter (groves + strays, family elevation bands, landmark exclusion zones), multi-mesh merge (canopies render; material arrays handled in reveal/dispose/audit). |
| 14. Concept-as-spine (wild-but-cohesive worlds) | ✅ Shipped | — | Sub-plan: `docs/plans/2026-06-10-003-feat-planet-visuals-phase-14-concept-spine-plan.md`. **Supersedes the archetype taxonomy** (Phase 12's parametric terrain/composition/creature machinery survives as the engine). One weird, *visible* premise per planet, generated at system spawn (Haiku `/concept`, replaces Tier 1 at the same call site): `{teaser, premise, tier, terrain, motif, question}` cached as the single source of truth — teaser-first exploration, Tier 2/3 elaborate instead of inventing. Spark dice for entropy without buckets; quiet/notable/singular tier dial (~60/33/7) controls coordination budget; five placement motifs (uniform-lean, all-facing-point, grid-rows, procession, shared-heading) + singular-tier overrides (embed-as-expression, cross-role slots). Voice spec: naturalist's journal, spoken not written. Quality bar: five worked reference planets in the sub-plan. Re-prioritizes Phase 7 (catalog breadth) + Phase 11 (thumbnail framing) as force multipliers. |
| 15. Surfaces: landforms, patterns, life, premise-faithful picks | 🔄 In review | [#32](https://github.com/kkoppenhaver/personal-space/pull/32) | Stacked on #30 (quick wins: terrain detail 5→16 = 720→5780 tris, planet-framing camera, concept registers/name initials) and #31 (rendering: sun-aimed key light, raymarched sun-lit atmospheres, glow sun, contrails). **15a landforms:** 13 terrain shapes + 7 ground patterns (`TerrainShapes.js`, `?terrainLab=1`), dealt without replacement per system and handed to the concept so premises are written on the landform; focal shapes host the hero; sea level is now a true surface quantile; exact altitude sampler; terrain assist lifts the plane over landforms. **15b legibility + fidelity:** surface 3.2-7m, footprint caps (wall-slab bug), candidate dedupe, hero-centred groves; hero pool widened past the 59 hero-role assets; MMR relevance normalized (diversity was dominating ~20×); BM25 stoplist + long-word-only fuzzy ("glass"→"grass"); multi-query retrieval; Poly Pizza for hero AND landmarks when the premise's object noun is missing, annotated for the picker. **15c life:** ambient motes by biome/premise (fireflies at night), animated seas. Worker concept prompt v4b (landform vocab, object keywords) uploaded as a preview version — deploy with the PR. |
