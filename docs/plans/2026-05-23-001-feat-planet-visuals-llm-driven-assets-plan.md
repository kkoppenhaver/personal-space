...
| 4. Planet construction refactor + cancellation tokens | ✅ Shipped | TBD | `Planet.applyVisuals()` binds hero/landmark/surface GLBs to slots via `AssetCache` + `MaterialSet`. `visualGen` token bumps on each call + on `dispose()`; in-flight loads check before mounting and bail silently on mismatch. `pickLandmarkSlots` expanded with spire + coast detection (≥3 hero-spaced slots guaranteed). Per-asset try/catch falls back to procedural marker per slot so a partial GLB failure never leaves a slot empty. `applyLLM(meta)` retained as palette-only shim. With empty catalog (v1 baseline), every code path no-ops cleanly to the procedural fallback. |
